...
export default class OrbitalService {
    ...
    static getEclipticGroupRotation = ({ longitudeOfAscendingNode, isSatellite }: { longitudeOfAscendingNode: number; isSatellite?: boolean }): THREE.Euler => {
    ...
    static getOrbitalGroupRotation = ({ inclination, argumentOfPeriapsis }: { inclination: number; argumentOfPeriapsis: number }): THREE.Euler => {
        return OrbitalService.toEuler({
            x: inclination,
    ...
    static getBodyRotation = ({ axialTilt, sidereal, time }: { axialTilt: number; sidereal: number; time: number }): THREE.Euler => {
        return OrbitalService.toEuler({
    ...
    static getBodyPosition = ({ periapses, time }: { periapses: Periapses; time: number }, ellipse: Ellipse): THREE.Vector3 => {
        return ellipse.getPosition(time, periapses);
    }

    static getPathOpacity = ({ id }: { id: string }, highlightedOrbitals: string[] | undefined, force?: boolean): number => {
        if (force) {
    ...
    static getTargetByName = (orbitals: OrbitalData[], targetName: string): OrbitalData | undefined => {
    ...
    static getOrbitalStats = (target: OrbitalData, time: number): OrbitalStats => {
        ...
        const { distance, trueAnomaly } = PhysicsService.getDistanceFromAttractingBody(
            eccentricity, time, periapses, semimajor
        );
