 ...
 * own. It is the first frame callback registered — see CanvasContent in SceneContainer.
 */
const advanceFrame = () => {
...
describe('App Container', () => {
  ...
  describe('render()', () => {
    it('should show the splash screen until the app data arrives', () => {
      const { container } = renderWithStore(<AppContainer action={action} />)
    ...
    it('should show the app once the data has arrived', () => {
      const { container } = renderWithStore(
        <AppContainer action={action} {...withData} />,
        initialState
      )
  ...
  describe('componentDidMount()', () => {
    it('should request the orbital data and page text', () => {
      renderWithStore(<AppContainer action={action} />)
  ...
  describe('onAnimate()', () => {
    it('should report each new time to the store while playing', () => {
      renderWithStore(<AppContainer action={action} {...withData} playing />, initialState)
    ...
    it('should not report a new time while paused', () => {
      renderWithStore(<AppContainer action={action} {...withData} playing={false} />, initialState)
    ...
    it('should apply the current speed to the clock', () => {
      renderWithStore(<AppContainer action={action} {...withData} speed={2} />, initialState)
  ...
  describe('maybeUpdateOffset()', () => {
    it('should move the running clock to a newly picked time', () => {
      const { rerender } = renderWithStore(
        <AppContainer action={action} {...withData} playing timeOffset={0} />,
        initialState
      )

      rerender(<AppContainer action={action} {...withData} playing timeOffset={123} />)
    ...
    it('should leave the clock alone while the simulation is paused', () => {
      const { rerender } = renderWithStore(
        <AppContainer action={action} {...withData} playing={false} timeOffset={0} />,
        initialState
      )

      rerender(<AppContainer action={action} {...withData} playing={false} timeOffset={123} />)
    ...
    it('should leave the clock alone when the time was not picked anew', () => {
      const { rerender } = renderWithStore(
        <AppContainer action={action} {...withData} playing timeOffset={123} />,
        initialState
      )
