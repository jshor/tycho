...
describe('Orbital Container', () => {
  ...
  describe('render()', () => {
    it('should render a group named after the orbital', () => {
      const { container } = render(<OrbitalContainer {...baseProps} />)
    ...
    it('should position the body along its orbit at the current time', () => {
      ...
      render(<OrbitalContainer {...baseProps} />)
  ...
  describe('body state', () => {
    it('should reposition the body as the clock ticks', () => {
      ...
      const { rerender } = render(<OrbitalContainer {...baseProps} />)

      spy.mockClear()
      rerender(<OrbitalContainer {...baseProps} time={2} />)
    ...
    it('should leave the body where it is when nothing has changed', () => {
      ...
      spy.mockClear()
      rerender(<OrbitalContainer {...baseProps} />)
    ...
    it('should reposition the body when the scale changes', () => {
      const spy = vi.spyOn(OrbitalService, 'getBodyPosition')
      const { rerender } = render(<OrbitalContainer {...baseProps} scale={1} />)

      spy.mockClear()
      rerender(<OrbitalContainer {...baseProps} scale={2} />)
  ...
  describe('scale', () => {
    it("should rescale a satellite's orbit when the scale changes", () => {
      ...
      const { rerender } = render(<OrbitalContainer {...baseProps} scale={1} isSatellite />)

      spy.mockClear()
      rerender(<OrbitalContainer {...baseProps} scale={2} isSatellite />)
    ...
    it('should rebuild the orbit path when the scale changes', () => {
      const { container, rerender } = render(
        <OrbitalContainer {...baseProps} scale={1} isSatellite />
      )
  ...
  describe('pathOpacity', () => {
    it('should recompute the path opacity when the highlighted orbitals change', () => {
      ...
      const { rerender } = render(<OrbitalContainer {...baseProps} highlightedOrbitals={[]} />)

      spy.mockClear()
      rerender(<OrbitalContainer {...baseProps} highlightedOrbitals={[orbital.id]} />)
    ...
    it('should mark the orbital as the target when the camera is focused on it', () => {
      ...
      render(<OrbitalContainer {...baseProps} targetId={orbital.id} />)
